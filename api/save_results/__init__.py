...
ERROR_IN_BODY   = os.getenv("ERROR_IN_BODY", "false").lower() == "true"
#   • DEBUG_MODE      = true  → on inclut la stack-trace dans la réponse
#   • ERROR_IN_BODY   = true  → on renvoie 200 au lieu de 500 pour que le
#                               message s’affiche directement dans le navigateur
...
def save_results(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if API_SECRET:
        if req.headers.get("x-api-secret") != API_SECRET:
            logging.warning("Secret incorrect ou manquant")
...
def _check_row_schema(row: dict) -> None:
    """
    Valide un élément de la liste `data`.
    """
...
def _cors_resp(code: int) -> func.HttpResponse:
    return func.HttpResponse(
        status_code=code,
        headers={
...
def _fatal(message: str) -> func.HttpResponse:
    """
    Erreur interne.
    • Si DEBUG_MODE=true  → on ajoute la stack-trace.
    • Si ERROR_IN_BODY=true → on renvoie 200 pour que le navigateur affiche
      le corps de la réponse (utile quand on ne peut pas consulter les logs).
    """
    body = {"status": 500, "error": message}
    if DEBUG_MODE:
        body["traceback"] = traceback.format_exc()

    # 200 ou 500 selon le mode choisi
    http_code = 200 if ERROR_IN_BODY else 500
