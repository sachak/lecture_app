...
(familiarisation + test 80 mots plein-écran)

Execution :  streamlit run exp3.py
...
# =============================================================================
# 3. CHARGEMENT DES FEUILLES + TIRAGE DES 80 MOTS
# =============================================================================
...
def sd_ok(sub, st_, fq_cols):
    ok_num = all(sub[c].std(ddof=0) <= st_[f"sd_{c}"] * SD_MULT[c] for c in NUM_BASE)
    ok_fqs = all(sub[c].std(ddof=0) <= st_[f"sd_{c}"] * SD_MULT["freq"] for c in fq_cols)
...
# =============================================================================
# 4. HTML / JS (fond noir + texte blanc)
# =============================================================================
HTML_TPL = Template(r"""
<!DOCTYPE html>
<html lang="fr">
...
#scr{font-size:60px;user-select:none;color:#fff;}
#ans{
  display:none;font-size:48px;width:60%;text-align:center;
...
function nextTrial(){
  ...
  let showDur=START,hideDur=CYCLE-showDur,tShow,tHide;
...
a.style.fontSize="32px";a.style.marginTop="30px";a.style.color="#fff";
document.body.appendChild(a);"""
    download_js = download_js.replace("$", "$$")

    # ---------- gestion plein-écran / starter ----------
    if fullscreen:
        starter_js = r"""
...
window.addEventListener("keydown",firstKey);"""
        fullscreen_js = ""  # déjà demandé au 1er key-press
    else:
    ...
    return HTML_TPL.substitute(
        ...
        FULLSCREEN=fullscreen_js,
        STARTER=starter_js
    )

# =============================================================================
# 5. GESTION DES ÉTATS STREAMLIT
# =============================================================================
for key, default in {
    "page": "intro",
    "tirage_ok": False,
...
if st.session_state.page == "intro":
    ...
    st.markdown("""
**Principe**  
Des mots très brefs apparaissent au centre, suivis d’un masque `########`.  

1. Fixez le centre de l’écran.  
2. Dès que vous reconnaissez le mot, appuyez sur **ESPACE**.  
3. Tapez le mot puis validez avec **Entrée**.

Déroulement : 2 mots d’entraînement puis 80 mots de test.
""")
...
# PAGE FAMILIARISATION -------------------------------------------------------
    ...
    components.html(
        experiment_html(PRACTICE_WORDS, with_download=False, fullscreen=False),
...
# PAGE TEST PRINCIPAL --------------------------------------------------------
    ...
    if not st.session_state.exp_started:
        ...
        st.markdown("Cliquez sur **Commencer le test** (la fenêtre passera en plein écran).")
