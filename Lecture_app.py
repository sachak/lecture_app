...
def sd_ok(sub: pd.DataFrame, st_: Dict, fq_cols: List[str]) -> bool:
    ...
    ok_fqs = all(sub[c].std(ddof=0) <= st_[f"sd_{c}"] * SD_MULT["freq"]
                 for c in fq_cols)
...
const hz=f*1000/(performance.now()-t0),ok=Math.abs(hz-60)<1.5;
r.textContent='≈ '+hz.toFixed(1)+' Hz';r.style.color=ok?'lime':'red';
