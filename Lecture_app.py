...
"""
EXPE 3 – Streamlit
• Intro  ➜  3 mots d’entraînement ➜  80 mots réels
• Le tirage des 80 mots est lancé en arrière-plan dès le chargement.
"""
import json, random, threading, time, pathlib, pandas as pd, numpy as np, streamlit as st
import streamlit.components.v1 as components
from stimuli_selector import get_stimuli          # ★ nouveau

# =============== 0. CONFIG STREAMLIT ===============
st.set_page_config(page_title="Expérience 3", layout="wide")
st.markdown("<style>#MainMenu,header,footer{visibility:hidden}</style>", unsafe_allow_html=True)

# =============== 1. CHARGEMENT DU LEXIQUE (pour les essais) ===============
...
def load_lexique() -> pd.DataFrame:
    ...
    df = df.rename(columns={"ortho": "word"})   # simplification
    df.word = df.word.str.upper()
    return df[["word"]].dropna()

LEX = load_lexique()

# =============== 2. DÉMARRER LE TIRAGE EN TÂCHE DE FOND =============== ★
def _launch_selection():
    """Fonction exécutée dans un thread -> stocke résultat dans session_state."""
    try:
...
if "stimuli_ready" not in st.session_state:
    ...
    threading.Thread(target=_launch_selection, daemon=True).start()   # lance !
...
if st.session_state.page == "intro":
    ...
    st.markdown("""
    Vous allez voir des suites de dièses « ######## » et de mots.
    Appuyez sur Espace dès que vous voyez le mot, puis tapez-le.
    Commençons par **3 essais d’entraînement** avec de petits mots.
    """)
...
# ---------- PAGE 2 bis : PATIENCE SI 80 MOTS PAS PRÊTS -------- ★
...
# ---------- PAGE 3 : EXPÉRIENCE RÉELLE ------------------------
    ...
    const W={json.dumps(STIMULI)},C=350,S=14,P=14;
    let i=0,res=[];
