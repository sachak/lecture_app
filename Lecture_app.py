...
<style> #MainMenu, header, footer {visibility:hidden;} button:disabled {opacity:.45!important;cursor:not-allowed!important;} </style>
...
XLSX             = Path(file).with_name("Lexique.xlsx")
N_PER_FEUIL_TAG  = 5
...
<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"/> <style> html,body{height:100%;margin:0;background:#000;color:#fff; display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center} #res{font-size:48px;margin:30px 0}button{font-size:24px;padding:8px 28px} </style></head><body> <h2>Test de fréquence : 60 Hz</h2> <div id="res">--</div><button id="go" onclick="mesure()">Démarrer</button> <script> function mesure(){ const res=document.getElementById('res'),btn=document.getElementById('go'); btn.disabled=true; res.textContent='Mesure en cours…'; res.style.color='#fff'; let f=0,t0=performance.now(); function loop(){ f++; if(f<120){ requestAnimationFrame(loop); } else{ const hz=f*1000/(performance.now()-t0), ok=hz>58&&hz<62; res.textContent='≈ '+hz.toFixed(1)+' Hz'; res.style.color = ok ? 'lime':'red'; btn.disabled=false; if(ok){ Streamlit.setComponentValue('ok'); } }} requestAnimationFrame(loop); } Streamlit.setComponentReady(); </script></body></html> """
...
if st.button("Passer à la présentation ➜"):
    go("intro" if p.hz_ok else "hz_fail")
0-bis. — page « hz_fail » si écran hors 60 Hz
elif p.page == "hz_fail":
st.error("Votre écran n’est pas réglé sur 60 Hz.")
st.markdown("""
L’expérience nécessite un affichage à 60 Hz.

• Réglez la fréquence dans les paramètres système
• ou connectez un moniteur 60 Hz
