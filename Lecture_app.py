...
main.block-container     {padding:0;}                    /* supprime la marge   */
...
# ────── (tout le code de tirage des stimuli : inchangé, voir plus bas) ───
...
def build_sheet()->pd.DataFrame:
    F,all_freq=load_sheets(),load_sheets()["all_freq_cols"]
...
# ──────────────────── composant HTML plein-écran helper ──────────────────
...
<div style="height:100%;display:flex;flex-direction:column;
            ...
            font-family:sans-serif;padding:0 4vw;box-sizing:border-box;gap:3vh">

  <h1 style="margin:0;font-weight:700;
             font-size:clamp(16px,3.8vmin,32px)">
  ...
  <div id="res" style="font-size:clamp(18px,6vmin,50px)">--</div>

  <div style="display:flex;gap:2vw;flex-wrap:wrap;justify-content:center">
    <button id="go"  onclick="mesure()"
            style="font-size:clamp(14px,3vmin,26px);
                   padding:.8em 2em;border-radius:8px">
      Démarrer
    </button>

    <button id="next" onclick="next()"
            style="display:none;font-size:clamp(14px,3vmin,26px);
                   padding:.8em 2em;border-radius:8px">
      Suivant&nbsp;➜
    </button>
  </div>
...
function mesure(){
  ...
  const loop=()=>{f++; if(f<120){requestAnimationFrame(loop);}else{
