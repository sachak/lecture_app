...
document.getElementById("go").onclick=()=>{
  ...
  b.disabled=true; res.style.color="#fff"; res.textContent="Mesure en cours…";
  let t=[],n=120;
  const step=k=>{
    t.push(k);
    if(t.length<n){requestAnimationFrame(step);}
      ...
      res.style.color=ok?"lime":"red"; b.disabled=false;
...
if st.session_state.page=="screen_test":
    ...
    if hz_val=="ok": st.session_state.hz_ok=True
...
# ═══════════════════ page 2 : FAMILIARISATION ═════════════════════════════
    ...
    # ——— iframe familiarisation ———
    components.html(
        Template(open(__file__).read()).substitute(),   # placeholder
