...
def build_sheet() -> pd.DataFrame:
    FEUILLES=load_sheets(); all_freq_cols=FEUILLES["all_freq_cols"]
...
def launch_background_tirage():
    def _worker():
        try:
            ...
            st.session_state.tirage_error=str(exc); st.session_state.tirage_ready=True; return
...
# ───────────── HTML EXPÉRIENCE ───────────── #
def experiment_html(words:list[str],with_download=True,cycle_ms=350,start_ms=14,step_ms=14)->str:
 ...
 a.style.marginTop="30px";document.body.appendChild(a);"""
    return f"""
<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"/>
<style>html,body{{height:100%;margin:0;display:flex;flex-direction:column;align-items:center;
justify-content:center;font-family:'Courier New',monospace}}#scr{{font-size:60px;user-select:none}}
...
function nextTrial(){{if(trial>=WORDS.length){{endExperiment();return}}const w=WORDS[trial],
 ...
 function onSpace(e){{if(e.code==="Space"&&active){{active=false;clearTimeout(tShow);
 clearTimeout(tHide);const rt=Math.round(performance.now()-t0);
 window.removeEventListener("keydown",onSpace);scr.textContent="";ans.style.display="block";
 ans.value="";ans.focus();function onEnter(ev){{if(ev.key==="Enter"){{ev.preventDefault();
...
if st.session_state.page=="intro":
    ...
    if st.session_state.tirage_ready:
        if st.session_state.tirage_error: placeholder.error(f"Erreur : {st.session_state.tirage_error}")
        else: placeholder.success("Stimuli du test prêts !")
    else:
    ...
    if not st.session_state.tirage_ready:
        st.warning("Les stimuli sont encore en cours de génération… Merci de patienter."); st.stop()
