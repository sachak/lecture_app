...
Le tirage est exécuté en tâche de fond sans toucher à l’API Streamlit.
"""
...
# ────────────────────────── utilitaire « rerun » ─────────────────────────── #
def _do_rerun():
    if hasattr(st, "rerun"):              # Streamlit ≥ 1.32
        st.rerun()
    else:                                 # versions plus anciennes
        st.experimental_rerun()

# ────────────────────── mise en page Streamlit (invariable) ───────────────── #
st.set_page_config(page_title="Expérience 3", layout="wide")
st.markdown("<style>#MainMenu,header,footer{visibility:hidden;}</style>",
            unsafe_allow_html=True)

# ───────────────────────── constantes de sélection des mots ───────────────── #
MEAN_FACTOR_OLDPLD = 0.40
MEAN_DELTA        = {"letters": .70, "phons": .70}
SD_MULTIPLIER     = {"letters": 2., "phons": 2.,
                     "old20": .25, "pld20": .25, "freq": 10.}
...
# ────────────────────── 1. chargement du classeur (main thread) ──────────── #
...
# ───────────────────── 2. fonctions SANS Streamlit pour le tirage ─────────── #
def masks(df, st_):
...
def build_sheet(FEUILLES):
    all_freq = FEUILLES["all_freq_cols"]
...
# ───────────────────── 3. thread de calcul (sans Streamlit) ───────────────── #
def _worker(feuilles, out_q: "queue.Queue"):
    try:
        df = build_sheet(feuilles)
        out_q.put(("ok", df))
    except Exception as exc:
        out_q.put(("err", str(exc)))

# ─────────────────────── 4. gestion de l’état Streamlit ───────────────────── #
for k,v in (("builder_started",False),("stimuli_ok",False),
            ("err_msg",""),("tirage_df",None),("STIMULI",None),
            ("result_q",queue.Queue()),("FEUILLES",None)):
...
def launch_background_build():
    if not st.session_state.builder_started:
        ...
        threading.Thread(target=_worker,
                         args=(st.session_state.FEUILLES,
                               st.session_state.result_q),
                         daemon=True).start()

def check_queue():
    q=st.session_state.result_q; changed=False
    while not q.empty():
        ...
        changed=True
    if changed: _do_rerun()

# ────────── 5. premier chargement des feuilles (dans le thread principal) ─── #
if st.session_state.FEUILLES is None:
    st.session_state.FEUILLES = load_sheets()
...
if st.session_state.page=="intro":
    ...
    if st.session_state.err_msg:
        st.error(st.session_state.err_msg); st.stop()
    ...
    if st.session_state.stimuli_ok:
        ...
        if st.button("Démarrer l’expérience"):
            st.session_state.page="exp"; _do_rerun()
    else:
...
# ------------------------- PAGE EXP --------------------------- #
    ...
    if not st.session_state.stimuli_ok: st.stop()
...
<script>
const WORDS = __WORDS__; const CYCLE = __CYCLE__; const START = __START__; const STEP = __STEP__;
let trial=0,results=[]; const scr=document.getElementById("scr"),ans=document.getElementById("ans");
function nextTrial(){ if(trial>=WORDS.length){endExperiment();return;}
 const w=WORDS[trial],mask="#".repeat(w.length); let show=START,hide=CYCLE-show,tS,tH,t0=performance.now(),active=true;
