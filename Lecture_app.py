...
• 60 / 120 Hz
• Responsive (PC, TV, mobile) – tap + clavier virtuel QWERTZ sur mobile
• Plein-écran : automatique grâce à allow_fullscreen (Streamlit ≥ 1 .25)
  + bouton ⛶ manuel dans l’iframe
"""
...
def html_fs(html: str, **kwargs):
    """
    Enveloppe components.html :
    – ajoute allow_fullscreen=True si l’argument existe (Streamlit ≥ 1.25)
    – retire key / allow_fullscreen si non supportés (versions plus anciennes)
    """
...
def sd_ok(s, st_, fq):
    return (
        s.nblettres.std(ddof=0) <= st_["sd_nblettres"] * SD_MULT["letters"]
        and s.nbphons.std(ddof=0) <= st_["sd_nbphons"] * SD_MULT["phons"]
        and s.old20.std(ddof=0) <= st_["sd_old20"] * SD_MULT["old20"]
        and s.pld20.std(ddof=0) <= st_["sd_pld20"] * SD_MULT["pld20"]
        and all(s[c].std(ddof=0) <= st_[f"sd_{c}"] * SD_MULT["freq"] for c in fq)
    )


def mean_lp_ok(s, st_):
    return (
        abs(s.nblettres.mean() - st_["m_nblettres"]) <= MEAN_DELTA["letters"] * st_["sd_nblettres"]
        and abs(s.nbphons.mean() - st_["m_nbphons"]) <= MEAN_DELTA["phons"] * st_["sd_nbphons"]
    )
...
def pick_five(tag, feuille, used, F):
    ...
    for _ in range(MAX_TRY_TAG):
        ...
        if (
            (tag == "LOW_OLD" and samp.old20.mean() >= st_["m_old20"] - MEAN_FACTOR_OLDPLD * st_["sd_old20"])
            or (tag == "HIGH_OLD" and samp.old20.mean() <= st_["m_old20"] + MEAN_FACTOR_OLDPLD * st_["sd_old20"])
            or (tag == "LOW_PLD" and samp.pld20.mean() >= st_["m_pld20"] - MEAN_FACTOR_OLDPLD * st_["sd_pld20"])
            or (tag == "HIGH_PLD" and samp.pld20.mean() <= st_["m_pld20"] + MEAN_FACTOR_OLDPLD * st_["sd_pld20"])
        ):
            continue
...
def build_sheet() -> pd.DataFrame:
    ...
    for _ in range(MAX_TRY_FULL):
        take = {sh: set() for sh in F if sh != "all_freq_cols"}
        groups, ok = [], True
        for tag in TAGS:
            bloc = []
            for sh in take:
        ...
        if ok:
            ...
            return df[
                ["ortho"] + NUM_BASE + ALL + ["source", "group", "old_cat", "pld_cat"]
            ]
    st.error("Impossible de générer la liste."); st.stop()


# ────── 3. template HTML (clavier virtuel + bouton ⛶) -------------------
HTML_TPL = Template(r"""
...
#scr{font-size:7vw;text-align:center;max-width:90vw;line-height:1.2;
...
#full{position:fixed;top:10px;right:10px;width:44px;height:44px;
 line-height:44px;font-size:28px;background:#444;color:#fff;border:none;
 border-radius:6px;opacity:.75;cursor:pointer}
#full:active{background:#666}
...
function nextTrial(){
 ...
 (function cross(){if(!active)return;
  if(++f>=CROSS_F)stim();else requestAnimationFrame(cross);} )();
 function stim(){
 ...
 function prompt(rt){
  ...
  else{ans.readOnly=false;setTimeout(()=>ans.focus(),50);}
  resizeAll();
  function enter(e){if(e.key==="Enter"){e.preventDefault();finishAnswer();}}
...
# ────── 4. composant test fréquence HTML --------------------------------
TEST_HTML = r"""
<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0"/>
<style>
...
# ────── 5. état de session ---------------------------------------------
defaults = dict(
    page="screen_test", tirage_ok=False, tirage_run=False,
