...
# ───────────────────────── CONFIG GÉNÉRALE ────────────────────────────────
st.set_page_config(page_title="Expérience - plein-écran", layout="wide")
st.markdown("""
<style>
...
# =============================================================================
# 2. HTML / JS PLEIN-ÉCRAN (fond noir, phases stimuli)
# =============================================================================
...
/* plein-écran déjà actif (bouton Streamlit), mais on vérifie : */
if(!parent.document.fullscreenElement){
   parent.document.documentElement.requestFullscreen().catch(()=>{});
}
...
function fpsOK(){
 return new Promise(ok=>{
   let t=[];function s(ts){t.push(ts);
     ...
     ok(1000/((t.slice(1).reduce((a,b,i)=>a+b-t[i],t[0]))/(t.length-1))); }
   requestAnimationFrame(s);
 });}
fpsOK().then(f=>{
  if(f<55||f>65){
     document.body.innerHTML='<div style=\"font-size:32px\">Écran '+f.toFixed(1)+' Hz : utilisez 60 Hz.</div>';
     return;
  }
  start();
});
...
function start(){
 let trial=0,results=[],scr=document.getElementById('scr'),
     ans=document.getElementById('ans');

 function next(){
   if(trial>=WORDS.length){fin();return;}
   let f=0;scr.textContent='+';
   const cross=()=>++f<$CROSS_FR?requestAnimationFrame(cross):show(WORDS[trial]);
   requestAnimationFrame(cross);
 }
 function show(w){
   const m='#'.repeat(w.length);
   let s=$SHOW_START,c=0,act=true,t0=performance.now();
   const loop=()=>{if(!act)return;
     scr.textContent=c<s?w:c<$CYCLE_FR?m:scr.textContent;
     if(++c==$CYCLE_FR){s+=$STEP_FR;c=0;}
     requestAnimationFrame(loop);};requestAnimationFrame(loop);
   const onSp=e=>{
     if(e.code==='Space'&&act){
       act=false;parent.document.removeEventListener('keydown',onSp);
...
a.style='font-size:32px;margin-top:30px';
document.body.appendChild(a);"""
    ...
    return HTML_TPL.substitute(
        ...
        END_MSG=json.dumps("Merci !"
                 if with_download else "Fin de l’entraînement"),
        DOWNLOAD=down)
...
if "go_fs"          not in st.session_state  : st.session_state.go_fs=False
...
• la page passera en **plein-écran** ;  
• la familiarisation démarrera automatiquement.  

(ESC ou F11 pour quitter le plein-écran à tout moment.)
""")

    # déclenche le tirage à la première ouverture
    ...
    if st.session_state.tirage_ok:
        if st.button("Cliquez ici pour passer en plein-écran et commencer"):
            st.session_state.page="fam"
            st.session_state.go_fs=True       # drapeau pour demander FS
            do_rerun()

# ─────────────────────── FAMILIARISATION ────────────────────────────────
elif st.session_state.page=="fam":
    # demande plein-écran une seule fois, juste après le clic utilisateur
    if st.session_state.go_fs:
        components.v1.html("""
           <script>parent.document.documentElement.requestFullscreen()
                    .catch(()=>{});</script>""",
           height=0, width=0)
        st.session_state.go_fs=False
