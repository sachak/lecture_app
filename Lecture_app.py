...
Tirage exécuté une fois, de manière synchrone, sous spinner.
...
function next(){{if(i>=WORDS.length){{fin();return;}}
  const w=WORDS[i],mask='#'.repeat(w.length);let sd=START,hd=CYCLE-sd,ts,th,
        t0=performance.now(),act=!0;
...
function fin(){{scr.style.fontSize='40px';scr.textContent='{end_msg}';{download_js}}}
next();
</script></body></html>"""
...
# ---- FAMILIARISATION ------------------------------------------------------ #
    ...
    get_tirage()   # ← calcule le tirage une seule fois
    st.header("Familiarisation (2 mots)")
    st.markdown("Appuyez sur **Espace** quand le mot apparaît, puis tapez ce que vous avez lu et validez avec **Entrée**.")
    components.v1.html(html_page(PRACTICE_WORDS, False, start_ms=250), height=650, scrolling=False)
    st.divider()
...
# ---- TEST PRINCIPAL ------------------------------------------------------- #
    ...
    components.v1.html(html_page(stimuli, True, start_ms=14), height=650, scrolling=False)
