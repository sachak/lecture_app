...
# =============================================================================
# 3. CHARGEMENT EXCEL + TIRAGE (identique aux versions précédentes)
# =============================================================================
...
def load_sheets()->dict[str,dict]:
    ...
    for sh in sheets:
        ...
        feuilles[sh]={"df":df,"stats":stats,"freq_cols":fqs}
...
def pick_five(tag,feuille,used,F):
    df,st_=F[feuille]["df"],F[feuille]["stats"]; fqs=F[feuille]["freq_cols"]
    pool=df.loc[masks(df,st_)[tag] & ~df.ortho.isin(used)]
    if len(pool)<N_PER_FEUIL_TAG: return None
    for _ in range(MAX_TRY_TAG):
        samp=pool.sample(N_PER_FEUIL_TAG,random_state=rng.randint(0,1_000_000))
...
#res{font-size:48px;margin:30px 0}button{font-size:24px;padding:8px 28px}</style></head><body>
<h2>Test fréquence écran (cible : 60 Hz)</h2>
...
document.getElementById("start").onclick=()=>{
  ...
  const step=k=>{
    t.push(k); if(t.length<n){requestAnimationFrame(step);}
      ...
      const ok=hz>58&&hz<62; res.style.color=ok?"lime":"red";
...
EXP_HTML=Template(r"""
<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"/>
<style>html,body{height:100%;margin:0;background:#000;display:flex;flex-direction:column;
align-items:center;justify-content:center;font-family:'Courier New',monospace}
#scr{font-size:60px;color:#fff;user-select:none}
#ans{display:none;font-size:48px;width:60%;text-align:center}</style></head>
...
function waitF(n,c){let i=0;function s(){i++>=n?c():requestAnimationFrame(s)}requestAnimationFrame(s)}
function present(){
 if(trial>=WORDS.length){fin();return;}
 const w=WORDS[trial],mask="#".repeat(w.length);let showF=START_F,hideF=CYCLE_F-showF;
 const t0=performance.now();let active=true;
 function cycle(){if(!active)return;scr.textContent=w;
   waitF(showF,()=>{if(!active)return;scr.textContent=mask;
     waitF(hideF,()=>{if(active){showF+=STEP_F;hideF=Math.max(0,CYCLE_F-showF);cycle();}});});}
 cycle();
 function onSp(e){if(e.code==="Space"&&active){
   active=false;const rt=Math.round(performance.now()-t0);
   window.removeEventListener("keydown",onSp);scr.textContent="";
   ...
   ans.addEventListener("keydown",function onEn(ev){
     if(ev.key==="Enter"){ev.preventDefault();
       ...
       ans.removeEventListener("keydown",onEn);ans.style.display="none";trial++;present();}});}}
...
window.addEventListener("keydown",first);"""
    ...
    return EXP_HTML.substitute(
        WORDS=json.dumps(words),CYCLE_F=cycle_frames,START_F=start_frames,STEP_F=step_frames,
        END_MSG=json.dumps("Merci !" if with_download else "Fin de l’entraînement"),
        DOWNLOAD=dl,STARTER=start)
...
""")
    ...
    if st.session_state.tirage_running and not st.session_state.tirage_ok:
        with st.spinner("Tirage aléatoire des 80 mots…"):
            df=build_sheet()
            l=df["ortho"].tolist(); random.shuffle(l)
            st.session_state.stimuli=l
