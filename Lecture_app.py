...
• 60 / 120 Hz
• Responsive (PC, TV, mobile)
• Mobile : tap + clavier virtuel QWERTZ (aucune suggestion)
• Plein-écran :
   – Streamlit ≥ 1.25 : allow_fullscreen + requestFullscreen automatique
   – Streamlit < 1.25 : bouton ⛶ dans l’iframe pour passer manuellement
"""
...
def html_fs(html: str, **kw):
    """
    Appelle components.html en :
      • ajoutant allow_fullscreen=True si l’argument existe,
      • retirant key / allow_fullscreen si non supportés (compat < 1.25)
    """
    sig = inspect.signature(components.html).parameters
    if "allow_fullscreen" in sig:
        kw.setdefault("allow_fullscreen", True)
    else:
        kw.pop("allow_fullscreen", None)
    if "key" not in sig:
        kw.pop("key", None)
...
#full{position:fixed;top:10px;right:10px;width:44px;height:44px;line-height:44px;
      font-size:28px;background:#444;color:#fff;border:none;border-radius:6px;
...
#  … TEST_HTML (déjà fourni ci-dessus) …
...
# 4. test principal -------------------------------------------------------
    ...
    if not p.exp_started:
        ...
        with st.expander("Aperçu du tirage (5 premières lignes)"):
