...
# ─────────────── configuration globale (plein-écran, zéro scroll) ────────
st.set_page_config(page_title="Expérience 3", layout="wide")
st.markdown(
    """
    <style>
      html,body,.stApp {height:100%; margin:0; overflow:hidden;}
      main.block-container {padding:0;}              /* retire marge interne   */
      #MainMenu,header,footer{visibility:hidden;}    /* cache UI Streamlit     */
...
p = st.session_state
def go(pg: str) -> None: p.page = pg; _rerun()


# ────────── constantes & fonctions de tirage (copiées de votre script) ───
...
def to_float(s: pd.Series) -> pd.Series:
    return pd.to_numeric(
        s.astype(str)
         .str.replace(r"[  ,]", "", regex=True)   # supprime espaces insécables
...
# ───────── helper : i-frame plein-écran qui se redimensionne à 100 % ─────
...
<div style="height:100%;display:flex;flex-direction:column;
  ...
  <div style="display:flex;gap:3vw;flex-wrap:wrap;justify-content:center">
    ...
    <!-- bouton visible mais désactivé tant que la mesure n’est pas OK -->
    <button id="next"
            style="font-size:clamp(14px,3vmin,24px);
                   padding:.7em 2em;border-radius:8px"
            disabled onclick="suivant()">Suivant&nbsp;➜</button>
  </div>
</div>
...
function mesure(){
  ...
  (function loop(){
      ...
      if (f < 120){ requestAnimationFrame(loop); }
          ...
          Streamlit.setComponentValue(hzVal);   // renvoie à Python
          go.disabled = false;
      }
  })();
}
...
function suivant(){
  /* valeur unique pour déclencher Streamlit à chaque clic */
...
# ──────────────────── PAGE 1 : présentation + tirage ─────────────────────
    ...
    st.markdown(
        ...
        <div style='height:100%;display:flex;flex-direction:column;
          ...
          <p style='max-width:650px;font-size:clamp(14px,3.5vmin,26px);
