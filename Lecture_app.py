# -*- coding: utf-8 -*-
"""
EXPÉRIENCE 3 – tirage des 80 mots en tâche de fond.
Le bouton « Passer au test » vérifie que les stimuli sont prêts avant d’agir.
...
# ─── raccourci « rerun » compatible toutes versions ───────────────────────
_rerun = st.rerun if hasattr(st, "rerun") else st.experimental_rerun

# ─── constantes / paramètres (identiques à vos specs) ─────────────────────
...
# ─── 1. lecture de Lexique.xlsx (cache) ───────────────────────────────────
...
# ─── 2. génération (lourde) de la liste de 80 mots ───────────────────────
...
# ─── 3. thread : pas de rerun automatique (évite crash pendant la fam) ───
def launch_thread():
    def worker():
        try:
            df = build_sheet()
        ...
        st.session_state.tirage_done = True   # pas de _rerun ici !
    th = threading.Thread(target=worker, daemon=True)
    add_script_run_ctx(th); th.start()
...
# ─── 4. template HTML/JS (aucune f-string) ───────────────────────────────
...
<script>
const WORDS=__WORDS__,CYCLE=__CYCLE__,START=__START__,STEP=__STEP__,DL=__DL__;
let trial=0,results=[],scr=document.getElementById("scr"),ans=document.getElementById("ans");
function nextTrial(){if(trial>=WORDS.length){end();return;}const w=WORDS[trial],mask="#".repeat(w.length);
let show=START,hide=CYCLE-show,tS,tH,active=true,t0=performance.now();
(function loop(){if(!active)return;scr.textContent=w;
tS=setTimeout(()=>{if(!active)return;scr.textContent=mask;
tH=setTimeout(()=>{if(active){show+=STEP;hide=Math.max(0,CYCLE-show);loop();}},hide);},show);})();
...
window.addEventListener("keydown",onSpace);}function end(){
scr.style.fontSize="40px";scr.textContent=DL?"Merci !":"Fin de l’entraînement";
if(!DL)return;const csv=["word;rt_ms;response",...results.map(r=>`${r.word};${r.rt_ms};${r.response}`)].join("\\n");
const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));
a.download="results.csv";a.textContent="Télécharger les résultats";a.style.fontSize="32px";a.style.marginTop="30px";
document.body.appendChild(a);}nextTrial();</script></body></html>"""
def make_html(words:list[str], with_download=True, cycle_ms=350, start_ms=14, step_ms=14)->str:
...
# familiarisation
    ...
    if st.session_state.tirage_done:
        if st.session_state.tirage_error:
            st.error(f"Erreur : {st.session_state.tirage_error}")
        else:
            st.success("Stimuli du test prêts !")
    else:
        st.info("Préparation des stimuli du test en arrière-plan…")
    ...
    if st.button("Passer au test principal"):
        if st.session_state.tirage_done and not st.session_state.tirage_error:
            st.session_state.page="exp"; _rerun()
        else:
            st.warning("Les stimuli ne sont pas encore prêts ; veuillez patienter.")
...
# test principal
    ...
    components.v1.html(make_html(st.session_state.stimuli, with_download=True),
                       height=650, scrolling=False)
